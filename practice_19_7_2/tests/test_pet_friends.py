...
def test_get_api_key_for_user_with_invalid_password(email=valid_email, password=invalid_password):
    """Проверяем, что запрос api ключа с неверным password возвращает статус 403"""
...
def test_add_new_pet_without_photo_and_with_valid_data(name='Джек', animal_type='собака', age='5'):
...
def test_add_new_pet_without_name(name='', animal_type='кошка', age='3',
                                  pet_photo='images/1.JPG'):
    """Проверяем возможность добавить питомца с пустым полем name"""
    ...
    assert status == 200
    assert result['name'] == name
def test_add_photo_for_pet_with_valid_data():
    """Проверяем возможность добавить фото питомцу с указанным id"""

    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")

    if len(my_pets['pets']) > 0:
        status, result = pf.post_add_photo_for_pet(auth_key, my_pets['pets'][0]['id'], pet_photo='images/2.JPG')
        assert status == 200
    else:
        raise Exception("There is no my pets")
